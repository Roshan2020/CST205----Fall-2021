# CST205 - Multimedia Design & Progamming
# Lab 3
# Fall 2021
# Author - Menik Arachchige, Roshan Indika Menik Arachchi

from PIL import Image # an error here means that Pillow is not installed

mona_string = '''132 128 126 123 137 129 130 145 158 170 172 161 153 158 162 172 159 152;
...
'''
# String above contains a group of data which represents the grayscale pixels and its postions

mona_lists = mona_string.splitlines() # the string should be divided after ever semi-colon (;)

# for loop is used next to eliminate every ; and create a list of strings using split
for count, mona_line in enumerate(mona_lists):
    mona_line_clean = mona_line.rstrip(';')
    mona_lists[count] = mona_line_clean.split()

# the code used below are specific intruction to display the img based on the input width and height
width = 18
height = 29
my_mona = Image.new('L', (width, height))
width, height = my_mona.size

for x in range(width):
    for y in range(height):
        my_mona.putpixel((x,y), int(mona_lists[y][x]))

# saves the img with the name 'mona.png'
